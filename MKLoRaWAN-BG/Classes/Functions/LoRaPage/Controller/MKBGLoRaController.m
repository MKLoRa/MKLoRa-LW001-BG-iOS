...
@interface MKBGLoRaController ()<UITableViewDelegate, UITableViewDataSource>

@property (nonatomic, strong)MKBaseTableView *tableView;

@property (nonatomic, strong)NSMutableArray *dataList;

@property (nonatomic, strong)MKBGLoRaPageModel *dataModel;

@end

@implementation MKBGLoRaController

- (void)dealloc {
    NSLog(@"MKBGLoRaController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 1) {
        //Connection Settings
        MKBGLoRaSettingController *vc = [[MKBGLoRaSettingController alloc] init];
...
- (MKBGLoRaPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBGLoRaPageModel alloc] init];
    }
    return _dataModel;
}
