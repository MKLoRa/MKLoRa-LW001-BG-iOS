...
MKBGLoRaSettingAccountCellDelegate>
...
- (void)rightButtonMethod {
    ...
    if (!ValidStr([MKBGUserLoginManager shared].password)) {
        //当前未登录
        @weakify(self);
        MKIoTCloudAccountLoginAlertViewModel *viewModel = [[MKIoTCloudAccountLoginAlertViewModel alloc] init];
        viewModel.account = [MKBGUserLoginManager shared].username;
        viewModel.isHome = [MKBGUserLoginManager shared].isHome;
        viewModel.password = [MKBGUserLoginManager shared].password;
    ...
    //登录过
    [self login:[MKBGUserLoginManager shared].isHome username:[MKBGUserLoginManager shared].username password:[MKBGUserLoginManager shared].password];
}
...
//        return (self.dataModel.configModel.supportClassType ? self.section3List.count : 0);
    ...
    if (section == 7) {
        //Account
        return ((self.dataModel.configModel.supportServerPlatform && self.dataModel.platform == 1 && ValidStr([MKBGUserLoginManager shared].password)) ? self.section6List.count : 0);
    }
    if (section == 8) {
        //Gateway EUI(Gateway ID)
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 2) {
        //mokoDevEUI
        MKBGDevEUICell *cell = [MKBGDevEUICell initCellWithTableView:tableView];
    ...
    if (indexPath.section == 7) {
        //Account
        MKBGLoRaSettingAccountCell *cell = [MKBGLoRaSettingAccountCell initCellWithTableView:tableView];
...
#pragma mark - MKBGLoRaSettingAccountCellDelegate
- (void)bg_loRaSettingAccountCell_logoutBtnPressed {
    MKIoTCloudExitAccountAlert *alert = [[MKIoTCloudExitAccountAlert alloc] init];
    BOOL isHome = [MKBGUserLoginManager shared].isHome;
    NSString *username = [MKBGUserLoginManager shared].username;
    [alert showWithAccount:username completeBlock:^{
        [[MKBGUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:@""];
...
- (void)login:(BOOL)isHome username:(NSString *)username password:(NSString *)password {
    ...
    [[MKIoTLoginService share] loginWithUsername:username password:password isHome:isHome sucBlock:^(id returnData) {
        ...
        [[MKBGUserLoginManager shared] syncLoginDataWithHome:isHome username:username password:password];
...
- (void)addDeviceToCloud:(NSString *)token {
    ...
    MKBGCreateLoRaDeviceModel *createModel = [[MKBGCreateLoRaDeviceModel alloc] init];
    createModel.macAddress = [MKBGConnectModel shared].macAddress;
    createModel.isHome = [MKBGUserLoginManager shared].isHome;
    createModel.gwId = self.dataModel.gatewayEUI;
    createModel.region = self.dataModel.region;
    createModel.username = [MKBGUserLoginManager shared].username;
    [[MKBGNetworkService share] addLoRaDeviceToCloud:createModel token:token sucBlock:^(id returnData) {
        [[MKHudManager share] hide];
        [self saveDataToDevice];
    } failBlock:^(NSError *error) {
        [[MKHudManager share] hide];
...
- (void)loadSection1Datas {
    MKBGDevEUICellModel *cellModel = [[MKBGDevEUICellModel alloc] init];
    NSString *macAddress = [[MKBGConnectModel shared].macAddress stringByReplacingOccurrencesOfString:@":" withString:@""];
...
- (void)loadSection6Datas {
    MKBGLoRaSettingAccountCellModel *cellModel = [[MKBGLoRaSettingAccountCellModel alloc] init];
    cellModel.account = [MKBGUserLoginManager shared].username;
