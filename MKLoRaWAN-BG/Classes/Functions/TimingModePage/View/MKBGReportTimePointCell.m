...
@implementation MKBGReportTimePointCellModel
@end

@interface MKBGReportTimePointCell ()
...
@implementation MKBGReportTimePointCell

+ (MKBGReportTimePointCell *)initCellWithTableView:(UITableView *)tableView {
    MKBGReportTimePointCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MKBGReportTimePointCellIdenty"];
    if (!cell) {
        cell = [[MKBGReportTimePointCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"MKBGReportTimePointCellIdenty"];
    }
    return cell;
}
...
- (void)deleteButtonPressed {
    if ([self.delegate respondsToSelector:@selector(bg_cellDeleteButtonPressed:)]) {
        [self.delegate bg_cellDeleteButtonPressed:self.dataModel.index];
    }
}
...
- (void)hourButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(bg_hourButtonPressed:)]) {
        [self.delegate bg_hourButtonPressed:self.dataModel.index];
    }
}
...
- (void)timeSpaceButtonPressed {
    ...
    if ([self.delegate respondsToSelector:@selector(bg_timeSpaceButtonPressed:)]) {
        [self.delegate bg_timeSpaceButtonPressed:self.dataModel.index];
    }
}

- (void)contentPanelTapAction {
    if (self.contentPanel.frame.origin.x == 0) {
        if ([self.delegate respondsToSelector:@selector(bg_cellTapAction)]) {
            [self.delegate bg_cellTapAction];
        }
        return;
    }
    if (self.contentPanel.frame.origin.x < 0){
...
- (void)swipeEventBeTriggered:(UISwipeGestureRecognizer *)swipeGesture{
    if ([self.delegate respondsToSelector:@selector(bg_cellResetFrame)]) {
        [self.delegate bg_cellResetFrame];
    }
...
#pragma mark - setter
- (void)setDataModel:(MKBGReportTimePointCellModel *)dataModel {
    _dataModel = nil;
