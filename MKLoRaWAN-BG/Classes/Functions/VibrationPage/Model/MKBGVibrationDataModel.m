...
@interface MKBGVibrationDataModel ()
...
@implementation MKBGVibrationDataModel
...
- (BOOL)readReportInterval {
    __block BOOL success = NO;
    [MKBGInterface bg_readVibrationDetectionReportIntervalWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configReportInterval {
    __block BOOL success = NO;
    [MKBGInterface bg_configVibrationDetectionReportInterval:[self.reportInterval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
...
- (BOOL)readVibrationTimeout {
    __block BOOL success = NO;
    [MKBGInterface bg_readVibrationTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configVibrationTimeout {
    __block BOOL success = NO;
    [MKBGInterface bg_configVibrationTimeout:[self.vibrationTimeout integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
