...
@interface MKBGDebuggerController ()<UITableViewDelegate,
UITableViewDataSource,
MFMailComposeViewControllerDelegate,
mk_bg_centralManagerLogDelegate,
MKBGDebuggerCellDelegate>
...
@property (nonatomic, strong)MKBGDebuggerButton *deleteButton;

@property (nonatomic, strong)MKBGDebuggerButton *exportButton;
...
@implementation MKBGDebuggerController

- (void)dealloc {
    NSLog(@"MKBGDebuggerController销毁");
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_bg_stopDebuggerMode" object:nil];
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [self loadSubViews];
    [MKBGCentralManager shared].logDelegate = self;
    [self readDataFromLocal];
    //进入debugger模式之后，即使设备断开连接，也要停留在当前页面
    [[NSNotificationCenter defaultCenter] postNotificationName:@"mk_bg_startDebuggerMode" object:nil];
...
- (void)leftButtonMethod {
    if ([MKBGCentralManager shared].connectStatus != mk_bg_centralConnectStatusConnected) {
        //当前设备已经断开连接，则返回扫描页面
    ...
    //当前设备正常连接，返回上一级页面
    self.leftAction = YES;
    [[MKBGCentralManager shared] notifyLogData:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MKBGDebuggerCell *cell = [MKBGDebuggerCell initCellWithTableView:tableView];
...
#pragma mark - mk_bg_centralManagerLogDelegate
...
#pragma mark - MKBGDebuggerCellDelegate
- (void)bg_debuggerCellSelectedChanged:(NSInteger)index selected:(BOOL)selected {
    ...
    MKBGDebuggerCellModel *cellModel = self.dataList[index];
...
- (void)deviceDisconnect {
    ...
    [self.syncIcon.layer removeAnimationForKey:@"mk_bg_syncAnimation"];
    self.isLoading = NO;
...
- (void)startButtonPressed {
    ...
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_delete_disableIcon.png");
    self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_export_disableIcon.png");
    [self.startButton setTitle:@"Stop" forState:UIControlStateNormal];
    [self.contentList removeAllObjects];
    [[MKBGCentralManager shared] notifyLogData:YES];
    [self.syncIcon.layer addAnimation:[MKCustomUIAdopter refreshAnimation:1.f]
                               forKey:@"mk_bg_syncAnimation"];
}

- (void)deleteButtonPressed {
    [[MKHudManager share] showHUDWithTitle:@"Waiting..." inView:self.view isPenetration:NO];
    [MKBGLogDatabaseManager deleteDatasWithMacAddress:self.macAddress sucBlock:^{
        NSMutableArray *tempList = [NSMutableArray array];
        for (NSInteger i = 0; i < self.dataList.count; i ++) {
            MKBGDebuggerCellModel *cellModel = self.dataList[i];
...
- (void)readDataFromLocal {
    ...
    [MKBGLogDatabaseManager readLocalLogsWithMacAddress:self.macAddress sucBlock:^(NSArray<NSDictionary *> * _Nonnull logList) {
        [[MKHudManager share] hide];
...
- (void)loadTableViewDatas:(NSArray *)logList {
    ...
    for (NSInteger i = 0; i < logList.count; i ++) {
        ...
        MKBGDebuggerCellModel *cellModel = [[MKBGDebuggerCellModel alloc] init];
...
- (void)saveDataListToLocal {
    ...
    [MKBGLogDatabaseManager insertLogDatas:tempList macAddress:self.macAddress sucBlock:^{
        [[MKHudManager share] hide];
        if (self.leftAction) {
...
- (void)updateTopButtonState {
    BOOL enable = NO;
    for (MKBGDebuggerCellModel *cellModel in self.dataList) {
        if (cellModel.selected) {
            enable = YES;
            break;
        }
    ...
    if (enable) {
        //选中了，可以删除或者导出
        self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_delete_enableIcon.png");
        self.exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_export_enableIcon.png");
        return;
    }
    //不存在选中的
    self.deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_delete_disableIcon.png");
...
- (UIImageView *)syncIcon {
    if (!_syncIcon) {
        ...
        _syncIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_sync_disableIcon.png");
    }
    return _syncIcon;
}

- (MKBGDebuggerButton *)deleteButton {
    if (!_deleteButton) {
        _deleteButton = [[MKBGDebuggerButton alloc] init];
        _deleteButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_delete_disableIcon.png");
...
- (MKBGDebuggerButton *)exportButton {
    if (!_exportButton) {
        _exportButton = [[MKBGDebuggerButton alloc] init];
        _exportButton.topIcon.image = LOADICON(@"MKLoRaWAN-BG", @"MKBGDebuggerController", @"bg_export_disableIcon.png");
