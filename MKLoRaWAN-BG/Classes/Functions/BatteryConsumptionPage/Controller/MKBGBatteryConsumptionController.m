//
//  MKBGBatteryConsumptionController.m
...
@interface MKBGBatteryConsumptionController ()<UITableViewDelegate,
...
@property (nonatomic, strong)MKBGBatteryConsumptionModel *dataModel;

@end

@implementation MKBGBatteryConsumptionController

- (void)dealloc {
    NSLog(@"MKBGBatteryConsumptionController销毁");
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        MKBGBatteryInfoCell *cell = [MKBGBatteryInfoCell initCellWithTableView:tableView];
...
- (void)sendBatteryResetCommandToDevice {
    [[MKHudManager share] showHUDWithTitle:@"Setting..."
                                     inView:self.view
                              isPenetration:NO];
    [MKBGInterface bg_batteryResetWithSucBlock:^{
        [[MKHudManager share] hide];
...
- (void)loadSection0Datas {
    MKBGBatteryInfoCellModel *cellModel = [[MKBGBatteryInfoCellModel alloc] init];
...
- (MKBGBatteryConsumptionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBGBatteryConsumptionModel alloc] init];
    }
    return _dataModel;
}
