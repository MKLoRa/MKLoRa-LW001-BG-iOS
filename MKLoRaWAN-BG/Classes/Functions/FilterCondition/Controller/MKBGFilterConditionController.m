//
//  MKBGFilterConditionController.m
...
@interface MKBGFilterConditionController ()
<UITableViewDelegate,
...
@implementation MKBGFilterConditionController

- (void)dealloc {
    NSLog(@"MKBGFilterConditionController销毁");
}
...
- (void)rightButtonMethod {
    ...
    if (self.dataModel.rawDataIson) {
        for (NSInteger i = 0; i < self.section3List.count; i ++) {
            ...
            MKBGFilterRawAdvDataModel *model = [[MKBGFilterRawAdvDataModel alloc] init];
...
#pragma mark - UI
- (void)loadSubViews {
    self.defaultTitle = (self.conditionType == mk_bg_conditionType_A) ? @"Filter Condition A" : @"Filter Condition B";
    [self.rightButton setImage:LOADICON(@"MKLoRaWAN-BG", @"MKBGFilterConditionController", @"bg_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBGFilterConditionModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBGFilterConditionModel alloc] init];
        _dataModel.isConditionA = (self.conditionType == mk_bg_conditionType_A);
    }
    return _dataModel;
}
