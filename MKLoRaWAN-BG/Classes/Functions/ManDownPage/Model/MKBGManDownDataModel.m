...
@interface MKBGManDownDataModel ()
...
@implementation MKBGManDownDataModel
...
- (BOOL)readIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKBGInterface bg_readIdleDetectionTimeoutWithSucBlock:^(id  _Nonnull returnData) {
        success = YES;
...
- (BOOL)configIdleDetectionTimeout {
    __block BOOL success = NO;
    [MKBGInterface bg_configIdleDetectionTimeout:[self.interval integerValue] sucBlock:^{
        success = YES;
        dispatch_semaphore_signal(self.semaphore);
    } failedBlock:^(NSError * _Nonnull error) {
        dispatch_semaphore_signal(self.semaphore);
