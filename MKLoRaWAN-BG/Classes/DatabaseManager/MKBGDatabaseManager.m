...
@implementation MKBGDatabaseManager

+ (BOOL)initDataBase {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"LoRaWANBGDB")];
    if (![db open]) {
        return NO;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists LoRaWANBGTable (date text,rawData text)"];
...
+ (void)insertDataList:(NSArray <NSDictionary *>*)dataList
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"LoRaWANBGDB")] inDatabase:^(FMDatabase *db) {
        for (NSDictionary *dic in dataList) {
            [db executeUpdate:@"INSERT INTO LoRaWANBGTable (date, rawData) VALUES (?,?)",SafeStr(dic[@"date"]),SafeStr(dic[@"rawData"])];
        }
        if (sucBlock) {
...
+ (void)readDataListWithSucBlock:(void (^)(NSArray <NSDictionary *>*dataList))sucBlock
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"LoRaWANBGDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM LoRaWANBGTable"];
        while ([result next]) {
...
+ (BOOL)clearDataTable {
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"LoRaWANBGDB")];
    if ([db open]) {
        BOOL success = [db executeUpdate:@"DELETE FROM LoRaWANBGTable"];
        [db close];
        return success;
    }
    return NO;
