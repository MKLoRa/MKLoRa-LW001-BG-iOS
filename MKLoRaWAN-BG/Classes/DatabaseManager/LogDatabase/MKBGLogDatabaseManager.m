...
@implementation MKBGLogDatabaseManager
...
+ (void)insertLogDatas:(NSArray <NSDictionary *>*)logList
    ...
    FMDatabase* db = [FMDatabase databaseWithPath:kFilePath(@"MKBGLogDB")];
    if (![db open]) {
        [self operationInsertFailedBlock:failedBlock];
        return;
    }
    NSString *sqlCreateTable = [NSString stringWithFormat:@"create table if not exists MKBGLogDataTable (macAddress text,date text,logDetails text)"];
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKBGLogDB")] inDatabase:^(FMDatabase *db) {
        
        for (NSDictionary *logDic in logList) {
            BOOL exist = NO;
            FMResultSet * result = [db executeQuery:@"select * from MKBGLogDataTable where macAddress = ? and date = ?",macAddress,SafeStr(logDic[@"date"])];
            while (result.next) {
                exist = YES;
            }
            if (exist) {
                //存在该设备，更新设备
                [db executeUpdate:@"UPDATE MKBGLogDataTable SET logDetails = ? where macAddress = ?",SafeStr(logDic[@"logDetails"]),macAddress];
            }else{
                //不存在，插入设备
                [db executeUpdate:@"INSERT INTO MKBGLogDataTable (macAddress,date,logDetails) VALUES (?,?,?)",macAddress,SafeStr(logDic[@"date"]),SafeStr(logDic[@"logDetails"])];
            }
        }
...
+ (void)readLocalLogsWithMacAddress:(NSString *)macAddress
    ...
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKBGLogDB")] inDatabase:^(FMDatabase *db) {
        ...
        FMResultSet * result = [db executeQuery:@"SELECT * FROM MKBGLogDataTable where macAddress = ?",macAddress];
        while ([result next]) {
            NSDictionary *dic = @{
...
+ (void)deleteDatasWithMacAddress:(NSString *)macAddress
                      ...
                      failedBlock:(void (^)(NSError *error))failedBlock {
    [[FMDatabaseQueue databaseQueueWithPath:kFilePath(@"MKBGLogDB")] inDatabase:^(FMDatabase *db) {
        BOOL result = [db executeUpdate:@"DELETE FROM MKBGLogDataTable where macAddress = ?",macAddress];
        if (!result) {
