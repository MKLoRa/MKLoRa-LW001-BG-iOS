...
static NSString *const mk_bg_logName = @"mk_bg_bleLog";
...
- (void)dealloc {
    NSLog(@"MKBGCentralManager销毁");
    [self logToLocal:@"MKBGCentralManager销毁"];
}

- (instancetype)init {
    if (self = [super init]) {
        [self logToLocal:@"MKBGCentralManager初始化"];
...
- (void)logToLocal:(NSString *)string {
    ...
    [MKBLEBaseLogManager saveDataWithFileName:mk_bg_logName dataList:@[string]];
}
